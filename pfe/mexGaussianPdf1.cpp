	...
	)
{
	...
	//io_timer = clock();
	if (nargin != 6)
		mexErrMsgTxt("6 parameters are required: input data, number of clusters, number of threads,initialized means, covariance matrix, and N!");
	if(!mxIsDouble(in[0]) || !mxIsDouble(in[1]) || !mxIsDouble(in[2])||!mxIsDouble(in[3]) || !mxIsDouble(in[4]) || !mxIsDouble(in[5]))
	...
	int estep1_threads = num_threads<num_clusters? num_threads:num_clusters;
	//int estep2_threads = num_threads<num_events? num_threads:num_events;

	double *inicenters, *inicov, *N;  //initialized centers and covariance
	int num_c;           
	...
	if(num_clusters != mxGetM(in[5]) || mxGetN(in[5])!=1 )
		mexErrMsgTxt("Wrong input N matrix");
	N = mxGetPr(in[5]);
	...
	out[0] = mxCreateNumericMatrix(num_events,num_clusters,mxDOUBLE_CLASS,mxREAL);   //
	...
	clusters.pi    = (double*)malloc(sizeof(double)*num_clusters);
	clusters.constant = (double*)malloc(sizeof(double)*num_clusters);
	//clusters.avgvar = (double*)malloc(sizeof(double)*num_clusters);
	clusters.means = (double*)malloc(sizeof(double)*num_clusters*num_dimensions);//mxGetPr(out[0]);
	clusters.R     = (double*)malloc(sizeof(double)*num_clusters*num_dimensions*num_dimensions);//mxGetPr(out[1]);
	clusters.Rinv  = (double*)malloc(sizeof(double)*num_dimensions*num_dimensions*num_clusters);
	clusters.memberships = mxGetPr(out[0]);
	...
	memcpy(clusters.N,N,num_clusters*sizeof(double));
	...
	mt_distribution(fcs_data_by_dimension, &clusters, num_dimensions, num_clusters, num_events, estep1_threads);
