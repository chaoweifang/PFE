...
if ~exist(fullfile(seism_root_dir,'lib'),'dir')
    error('Error installing the package, the folder "lib" not found')
end
if ~exist(fullfile(seism_root_dir,'src'),'dir')
    error('Error installing the package, the folder "src" not found')
end

% Install own lib
addpath(seism_root_dir);
addpath(fullfile(seism_root_dir,'lib'));
%addpath(fullfile(root_dir,'cvpr2013'));
addpath(genpath(fullfile(seism_root_dir,'src')));
...
if exist('mex_eval_segms')~=3 %#ok<EXIST>
    error('The needed function (mex_eval_segms) not found. Have you build the package properly?')
end
